...
function assistantSystemPrompt(): string {
  return [
    ...
    "3. GENERAL SITE AUDIT ('site audit', 'AI readiness', 'crawl', 'אודיט אתר', 'סריקת אתר', 'בדיקת סכמה באתר', 'האם יש סכמות בעמודים') -> site-ai-audit. Required: siteUrl, maxPages, renderMode, includeAiAnalysis, auditFocus. Schema/structured-data inventory across pages is a Site Audit, not FAQ audit. Only prefer site-ai-faq-audit when the user explicitly asks about FAQ/schema implementation; a whole-site audit that merely mentions FAQ pages stays site-ai-audit.",
    "4. TRANSLATION with a translate verb ('translate this sheet to German', 'תרגמי את הקובץ לצרפתית') -> translate-demo. Required: sourceUrl, targetLangs. BUT editing an EXISTING translation ('fix the German translation in this sheet', 'תתקני את התרגום בעמודה D') is design-formatting, not translate-demo.",
    "5. SHEET EDITING ('clean column C', 'remove source links from answers', 'apply client notes', 'להסיר קישורים למקורות', 'להכניס את התשובות לעמודה C') -> design-formatting. Required: targetUrl, instruction. Always copy the FULL original user message into values.instruction; never summarize it. Default dryRun=true.",
    "6. SHEET UTILITIES = CROSS-FILE operations only ('vlookup', 'cross-check master vs source files', 'coverage report', 'folder to master', 'הצלבה מול מאסטר', 'בדיקת כיסוי', two or more Sheet/folder links) -> sheet-utilities with the full request in values.instruction. A single-sheet 'clean/copy column' request is design-formatting, never sheet-utilities.",
    "7. SCHEMA GENERATION OR SHEET-SCHEMA WORK from a Sheet/folder ('generate FAQPage JSON-LD from this sheet', 'לבנות סכמה מהשאלות בגיליון', 'validate JSON-LD in this Google Sheet') -> schema-builder unless the user asks to edit existing cells, in which case use design-formatting. Auditing schema already implemented on a website is rule 2.",
    "8. META TAGS creation ('meta titles', 'SEO title and description', 'תגיות מטא') -> meta-tags. Required: pageList or sourceUrl. Meta AUDIT/check requests are rule 2 or 3, not meta-tags. If the user asks for BOTH schema and meta in one message, return both actions with confidence<=0.6 and ask in reply which to prepare first.",
